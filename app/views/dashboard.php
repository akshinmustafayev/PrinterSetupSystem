			<div class="jarallax" style="background-image: url(app/views/img/jarallax/image1.jpg);margin-top: -20px;">
				<div class="jarallax-table">
					<div class="jarallax-table-cell">
						<h1 style="color: #fff;font-size:50px;"><?php echo $lang['BRAND']; ?></h1>
						<h3 style="color: #fff;"><?php echo $lang['DASHBOARD_TEXT_1']; ?></h3>
						<br>
						<p><a class="btn btn-danger btn-lg" href="index.php?branches" role="button"><?php echo $lang['DASHBOARD_INSTALL']; ?> »</a></p>
					</div>
				</div>
			</div>
			<div class="jarallax-gap" style="background-color:#272333;">
				<h3><?php echo $lang['DASHBOARD_TEXT_2']; ?></h3>
			</div>
			<div class="jarallax" style="background-image: url(app/views/img/jarallax/image2.jpg);"></div>
			<div class="jarallax" style="background-image: url(app/views/img/jarallax/image3.jpg);"></div>
			<div class="jarallax" style="background-image: url(app/views/img/jarallax/image4.jpg);"></div>
			<div class="jarallax" style="background-image: url(app/views/img/jarallax/image5.jpg);"></div>
			<div class="jarallax" style="background-image: url(app/views/img/jarallax/image6.jpg);"></div>
			<div class="jarallax" style="background-image: url(app/views/img/jarallax/image7.jpg);"></div>
			<div class="jarallax-gap" style="background-color:#222222;">
				<p>
					Akshin Mustafayev
				</p>
				<small>
					<?php echo $lang['DASHBOARD_FOOTER']; ?> <a href="mailto:akshin.mustafayev@unibank.az">Akshin Mustafayev</a>
				</small>
			</div>